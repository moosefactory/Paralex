...
final class TestParameterGroup: XCTestCase {
    ...
    func test_01_Group() throws {
        ...
        let groupB = ParametersGroup(identifier: Identifier(rawValue: "groupB", role: .group), parameters: [
            try Identifier(rawValue: "pB1", role: .parameter).makeParameter(),
            try Identifier(rawValue: "pB2", role: .parameter).makeParameter()
        ])

        let groupD = ParametersGroup(identifier: Identifier(rawValue: "groupD", role: .group), parameters: [
            try Identifier(rawValue: "pD1", role: .parameter).makeParameter(),
            try Identifier(rawValue: "pD2", role: .parameter).makeParameter()
        ])

        let groupC = ParametersGroup(identifier: Identifier(rawValue: "groupC", role: .group), parameters: [
            try Identifier(rawValue: "pC1", role: .parameter).makeParameter(),
            try Identifier(rawValue: "pC2", role: .parameter).makeParameter()
        ], subGroups: [groupD])

        let groupA = ParametersGroup(identifier: Identifier(rawValue: "groupA", role: .group), parameters: [
            try Identifier(rawValue: "pA1", role: .parameter).makeParameter(),
            try Identifier(rawValue: "pA2", role: .parameter).makeParameter()
